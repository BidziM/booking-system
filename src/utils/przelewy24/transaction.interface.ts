import { Encoding, Channel, Language, Country, Currency } from './enums';

export interface TransactionClass {
  createTransaction(order: Order);
}
...
export type Order = {
  /**
   * Time to expire the payment link (Value in minute)
   *
   * @type {number}
   * @memberof Order
   */
...
export interface VerifyBySession {
  orderId?: number;
  sessionId?: string;
  status: number;
  amount?: number;
